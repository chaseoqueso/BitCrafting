...
public class ItemBitPickaxe extends ItemPickaxe implements IItemBitTool {
        ...
        if ((double)state.getBlockHardness(worldIn, pos) != 0.0D && stack.hasTagCompound())
        {
            ...
            if(itemData.hasKey("EffectArray"))
            {
                ...
                for(int i = 0; i < effectlist.tagCount(); ++i)
                {
                    ...
                    {
                        activateEffect(effectData.getString("effect"), effectData.getFloat("power"), pos, state, blockDestroyer, blockDestroyer.world);
                    }
                }
            }
        }
    ...
    public void activateEffect(String effect, float power, BlockPos pos, IBlockState state, EntityLivingBase blockDestroyer, World world)
    {
        switch(effect)
        {
            case "fire":
