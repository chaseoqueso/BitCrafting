...
public class BlockBitFusionTable extends BlockContainer {
	...
	public BlockBitFusionTable(Material material) {
		super(material);
		setUnlocalizedName("BlockBitFusionTable");
		setRegistryName(new ResourceLocation(BitCraftingMod.MODID, "blockbitfusiontable"));
	...
	{
		player.openGui(BitCraftingMod.instance, 3, world, position.getX(), position.getY(), position.getZ());
		return true;
	}
	...
	{
		return Item.getItemFromBlock(BitCraftingBlocks.BLOCKS.blockBitFusionTable);
	}

	@Override
	public ItemStack getItem(World world, BlockPos position, IBlockState state)
	{
		return new ItemStack(BitCraftingBlocks.BLOCKS.blockBitFusionTable);
	}
	...
	public void onBlockPlacedBy(World world, BlockPos position, IBlockState state, EntityLivingBase placer, ItemStack itemStack)
	{
		if(itemStack.hasDisplayName())
			((TileEntityBitFusionTable) world.getTileEntity(position)).setFusionTableName(itemStack.getDisplayName());
	}

	@Override
	public void breakBlock(World world, BlockPos position, IBlockState state)
	{
		TileEntityBitFusionTable tileentity = (TileEntityBitFusionTable) world.getTileEntity(position);
