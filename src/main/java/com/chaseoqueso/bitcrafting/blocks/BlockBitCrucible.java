...
public class BlockBitCrucible extends BlockContainer {
	...
	public BlockBitCrucible(Material material) {
		super(material);
		setUnlocalizedName("BlockBitCrucible");
		setRegistryName(new ResourceLocation(BitCraftingMod.MODID, "blockbitcrucible"));
	...
	{
		return Item.getItemFromBlock(BitCraftingBlocks.BLOCKS.blockBitCrucible);
	}

	@Override
	public ItemStack getItem(World world, BlockPos position, IBlockState state)
	{
		return new ItemStack(BitCraftingBlocks.BLOCKS.blockBitCrucible);
	}
	...
	public static void updateBlockState(boolean burning, World world, BlockPos position)
	{
		...
		if(burning)
			world.setBlockState(position, BitCraftingBlocks.BLOCKS.blockBitCrucible.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true), 3);
		else
			world.setBlockState(position, BitCraftingBlocks.BLOCKS.blockBitCrucible.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, false), 3);
